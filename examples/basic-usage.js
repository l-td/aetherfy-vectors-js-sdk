...
async function basicUsageExample() {
  ...
  try {
    ...
    searchResults.forEach((result, index) => {
      ...
      console.log(`   Category: ${result.payload.category}, Price: $${result.payload.price}`);
    });

    // Retrieve specific products by ID
    console.log('Retrieving specific products...');
    const retrieved = await client.retrieve(collectionName, ['product_1', 'product_3'], {
      withPayload: true,
      withVectors: false
    });
    ...
    const collectionStats = await client.getCollectionAnalytics(collectionName, '1h');
    ...
    console.log(`- Avg search latency: ${collectionStats.avgSearchLatencyMs}ms`);
    ...
    collections.forEach(collection => {
      console.log(`- ${collection.name} (${collection.config.size}d, ${collection.config.distance})`);
    });
